...
abstract contract RegistrarRules is Ownable {
  ...
  /// @notice Renewal fee for names claimed by self verification (is USD cents);
  uint256 private selfVerifiedFee;
  ...
  event LabelPricesChanged(
    uint256[] lengths,
    uint256[] prices,
    bool clearPrevious
  );

  /// @notice Emitted when self verified fee changes
  ///
  /// @param newSelfVerifiedFee New self verified renewal fee
  event SelfVerifiedFeeChanged(uint256 newSelfVerifiedFee);
  ...
  /// @notice Sets the renewal fee for self verified names.
  ///
  /// @param _selfVerifiedFee Fee in USD cents (e.g., 500 = $5.00, 1 = $0.01).
  function setSelfVerifiedFee(uint256 _selfVerifiedFee) external onlyOwner {
    selfVerifiedFee = _selfVerifiedFee;
    emit SelfVerifiedFeeChanged(_selfVerifiedFee);
  }
